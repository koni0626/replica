...
def upload(project_id: int):
    ...
    - 保存先: ./uploads/<user_id>/<project_id>/<uuid>_<secure_name>
    - 返却: { ok, files: [{name, size, ext, text_preview, text_path}] }
      text_path は将来の再利用用（このAPIでは使わない）。
    ...
    for fs in request.files.getlist('files'):
        ...
        if not _ext_ok(fname):
            results.append({"name": fname, "ok": False, "error": "unsupported_extension"})
            continue
        fs.stream.seek(0, os.SEEK_END)
        size = fs.stream.tell()
        fs.stream.seek(0)
        if size > MAX_UPLOAD_BYTES:
            results.append({"name": fname, "ok": False, "error": "too_large"})
            continue
