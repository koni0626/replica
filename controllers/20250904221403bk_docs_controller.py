...
def upload(project_id: int):
    ...
    - 本APIは「抽出」までは行わず、まずはテキストとして読み取りを試みて先頭500文字をプレビューとして返す。
    """
    ...
    for fs in request.files.getlist('files'):
        ...
        # 簡易テキスト化: UTF-8優先、失敗時はignore（プレビュー用途のみ）
        text_content = ''
        try:
...
def _build_attachments_text(project_id: int, user_id: int, paths: list[str], per_file_limit: int = 100_000) -> str:
    ...
    - 各ファイル先頭 per_file_limit バイトまで（UTF-8 decode, errors='ignore'）
    ...
    for p in paths:
        try:
            ...
            snippet = data[:per_file_limit].decode('utf-8', errors='ignore')
            parts.append(f"\n\n---\n[添付ファイル:{name}]\n{snippet}")
        except Exception:
            # 読み取りに失敗したファイルはスキップ
            continue
