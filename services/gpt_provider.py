...
class GptProvider(object):
    def __init__(
        ...
        self.llm_with_tool = self.llm.bind_tools([
            ...
            tools.list_files,
            tools.make_dirs,
            tools.backup_then_write,
            tools.file_stat,
            tools.read_file_range,
            tools.list_python_symbols,
            tools.apply_text_edits_with_backup,
        ])
    ...
    def _build_messages(
        ...
        *,
        ...
        messages.append(SystemMessage(content=(
            ...
            "* ソースコードが見つからない場合は、その旨を回答に含めてください。ユーザーがソースを見て回答したのか、憶測で回答したのかをわかるようにしたい"
            "* 既存のソースコードを上書き・修正する場合は、必ず backup_then_write ツールでバックアップを取得してから書き込みを行ってください。\n"
            "* 新規ファイルの作成のみ write_file を使用して構いません。"
            "* 大きなファイルは絶対に全文を貼り付けたり読み込んだりしないこと。まず file_stat で規模を確認し、list_python_symbols で範囲を特定、必要箇所のみ read_file_range で取得し、編集は apply_text_edits_with_backup で find/replace 差分を適用すること。"
        )))
    ...
    def stream_with_history_and_tool(
        ...
        for _ in range(max_tool_turns):
            ...
            if not tool_calls:
                ...
                for chunk in self.llm.stream(conversation):
            ...
            for call in tool_calls:
                ...
                tool_call_count += 1  # ツール呼び出し回数をインクリメント
                print(f"Tool called {tool_call_count} times")  # 現在のツール呼び出し回数を出力
    ...
    def stream_with_history(
        ...
        """
        履歴+ナレッジを注入してストリーミングで応答を返す。
        enable_tools=True のとき、LLMが tool_calls を返す限りツールを実行して会話を進め、
        最終的なアシスタント文になったところでテキストを逐次 yield する。

        戻り値: ジェネレータ（strの断片を yield）
        """
        # 1) ベースメッセージ作成（既存ロジックを利用）
        messages = self._build_messages(
            project_id, prompt, svc,
            history_limit=history_limit,
            use_knowledge=use_knowledge,
    ...
    def generate_project_with_tools(
        ...
        *,
        ...
        tools_local = [_set_project_root, _write_file, _write_binary_file, _list_files, _finalize_zip]
        llm_with_tools = self.llm.bind_tools(tools_local)
    ...
    def refactor_with_repo_tools(
        ...
        def _repo_write(relpath: str, content: str) -> str:
            """バックアップを取得してから安全に上書きします（backup_then_writeを内部で使用）。"""
            try:
                abs_path = str(sandbox._safe(relpath))
                ok = tools.backup_then_write.invoke({"file_path": abs_path, "content": content})
                return f"backup_then_write={ok} path={relpath}"
            except Exception as e:
        ...
        # 追加: 大容量ファイル操作用の補助ツール（部分取得・シンボル一覧・部分編集）
        @tool("repo_stat")
        def _repo_stat(relpath: str) -> str:
            try:
                abs_path = str(sandbox._safe(relpath))
                return tools.file_stat.invoke({"file_path": abs_path})
            except Exception as e:
                return f"error={type(e).__name__}: {e}"

        @tool("repo_read_range")
        def _repo_read_range(relpath: str, start_line: int, end_line: int) -> str:
            try:
                abs_path = str(sandbox._safe(relpath))
                return tools.read_file_range.invoke({"file_path": abs_path, "start_line": start_line, "end_line": end_line})
            except Exception as e:
                return f"error={type(e).__name__}: {e}"

        @tool("repo_list_python_symbols")
        def _repo_list_python_symbols(relpath: str) -> str:
            try:
                abs_path = str(sandbox._safe(relpath))
                return tools.list_python_symbols.invoke({"file_path": abs_path})
            except Exception as e:
                return f"error={type(e).__name__}: {e}"

        @tool("repo_apply_text_edits")
        def _repo_apply_text_edits(relpath: str, edits_json: str) -> str:
            try:
                abs_path = str(sandbox._safe(relpath))
                return tools.apply_text_edits_with_backup.invoke({"file_path": abs_path, "edits_json": edits_json})
            except Exception as e:
                return f"error={type(e).__name__}: {e}"

        tools_local = [_repo_list, _repo_read, _repo_write, _repo_patch, _repo_stat, _repo_read_range, _repo_list_python_symbols, _repo_apply_text_edits]
        llm_with_tools = self.llm.bind_tools(tools_local)
            ...
            for call in tool_calls:
                ...
                try:
                    if name == "repo_list":
                        ...
                        result = _repo_stat.invoke(args)
                    elif name == "repo_read_range":
                        result = _repo_read_range.invoke(args)
                    elif name == "repo_list_python_symbols":
                        result = _repo_list_python_symbols.invoke(args)
                    elif name == "repo_apply_text_edits":
                        result = _repo_apply_text_edits.invoke(args)
                    else:
                        result = f"error=unknown tool {name}"
                except Exception as e:
                    result = f"error={type(e).__name__}: {e}"
