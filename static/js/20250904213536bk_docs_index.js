...
(function(){
  ...
  window.addEventListener('DOMContentLoaded', () => {
     ...
     * 成功時は attachments-list にチップを追加。アップロード内容（テキスト化済みプレビュー）は
     * LLMへ渡すコンテキストとして、プロンプト末尾に追記する運用（初期実装）。
     */
    (function(){
      ...
      async function doUpload(files){
        ...
        // UI反映とプロンプト末尾への添付要旨追記（初期は先頭数百文字のみ）
        const parts = [];
        ...
        if (parts.length && promptInput){
          promptInput.value = (promptInput.value || '') + parts.join('');
          const evt = new Event('input');
          promptInput.dispatchEvent(evt);
        }
      }
